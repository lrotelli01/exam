...
In our case, residuals remain centered around zero, but the spread increases at higher predicted throughput (funnel shape). This indicates \textbf{moderate heteroskedasticity}, typical when the system approaches saturation and queueing dynamics amplify variability.

Implications:
\begin{itemize}
    \item Point predictions are still useful (no strong systematic bias in the mean residual).
    \item Uncertainty is higher in high-load configurations.
    \item Inference assuming constant variance can be optimistic; robust errors or weighted models are preferable for strict statistical testing.
\end{itemize}
