...
The analysis was performed using data from the \textbf{consistency test} simulations:

\begin{itemize}
    \item Configuration: $N = 1000$ users, $M = 10$ tables, $p = 0.5$, $\lambda = 0.05$ req/s, $S = 0.1$s
    \item 3 independent replications with different random seeds
    \item Response variable: Mean wait time (ms)
\end{itemize}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{images/warmup_analysis.pdf}
\caption{Warm-up analysis for N=1000 configuration. Top-left: wait time across replications. Top-right: normalized convergence to steady state. Bottom-left: steady-state wait time by user count. Bottom-right: coefficient of variation showing system stability.}
\label{fig:warmup_analysis}
\end{figure}

\subsubsection{Key Findings}

The analysis [Figure~\ref{fig:warmup_analysis}] reveals:

\begin{itemize}
    \item \textbf{Rapid Convergence}: The system reaches steady state quickly, with all replications showing similar wait times (mean = 31.34 ms, CV = 1.1\%).
    
    \item \textbf{Low Variability}: The coefficient of variation below 10\% indicates stable system behavior, suggesting a short warm-up period is sufficient.
    
    \item \textbf{Scaling Behavior}: Wait time increases with user count, as expected from queueing theory.
\end{itemize}

\subsubsection{Warm-Up Period Estimation}

Based on the low variability observed (CV = 1.1\%), the system converges rapidly to steady state:

\begin{table}[H]
\centering
\begin{tabular}{|c|c|c|c|}
\hline
\textbf{Configuration} & \textbf{Mean Wait Time} & \textbf{Std Dev} & \textbf{CV} \\
\hline
N=1000, M=10, p=0.5 & 31.34 ms & 0.34 ms & 1.1\% \\
\hline
\end{tabular}
\caption{Warm-up analysis results for reference configuration}
