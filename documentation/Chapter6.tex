...
The inter-arrival time between consecutive requests from each user follows an \textbf{exponential distribution} with rate $\lambda$:
\begin{equation}
T \sim \text{Exp}(\lambda), \quad f(t) = \lambda e^{-\lambda t}, \quad E[T] = \frac{1}{\lambda}
\end{equation}

This memoryless property is essential for the Markovian analysis of the system.
...
\begin{itemize}
    \item Configuration A (N=100): mean throughput $\approx 5.0$ req/s
    \item Configuration B (N=101): mean throughput $\approx 5.05$ req/s
    \item Variation: $\approx +1\%$
\end{itemize}

The 95\% confidence intervals of the two configurations \textbf{overlap}, confirming that the small input change produces a correspondingly small output change. The \textbf{continuity test passes}.
...
\begin{enumerate}
    ...
    \item \textbf{Accurate}: Theoretical predictions match empirical results within 5\%
\end{enumerate}
