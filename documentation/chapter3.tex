...
The following modules have been defined in OMNeT++:

\begin{itemize}
    \item \textbf{User}: A simple module representing database clients. Each user generates access requests according to an exponential inter-arrival time ($\lambda$), selects tables based on a configurable distribution (uniform or lognormal), and performs either read or write operations with probability $p$.
    
    \item \textbf{Table}: A simple module representing database tables. Each table implements a readers-writers lock mechanism allowing multiple concurrent readers but exclusive writer access. Tables record utilization, queue length, and waiting time statistics.
    
    \item \textbf{DatabaseNetwork}: A compound module (network) that instantiates N users and M tables with full connectivity between them.
\end{itemize}
...
\hline
User & \texttt{lambda} & Request rate (requests/second) \\
...
The simulation uses OMNeT++'s \texttt{cMessage} class extended with custom fields:

\begin{itemize}
    \item \textbf{Request Message}: Contains the user ID, target table, operation type (read/write), and request timestamp for response time calculation.
    
    \item \textbf{Response Message}: Contains the completion status and is used to signal the user that the operation has finished.
\end{itemize}

Tracking the initiation time of each request is crucial for calculating the response time and waiting time statistics.
...
The results show that adding just 1 user (+1\%) produces proportionally small changes in all metrics:
\begin{itemize}
    \item Throughput: +1.0\%
    \item Waiting Time: +1.1\%
    \item Utilization: +0.9\%
\end{itemize}

Since all variations are small and proportional to the input change, the \textbf{continuity test passes}.
...
Given that our system can be modeled as a \textbf{Closed Queueing Network} with think time, we can theoretically compute its performance indices.

\subsubsection{System Model}

The system is modeled as an interactive system (machine-repairman model) with:
\begin{itemize}
    \item $M = 10$ service centers (database tables)
    \item $K = N$ customers (users)
    \item $Z = 1/\lambda = 20$s think time
    \item $S = 0.1$s service time
\end{itemize}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{images/interactive_system_model.pdf}
\caption{Interactive system model: users cycle between thinking and requesting service}
\label{fig:interactive_model}
\end{figure}

\subsubsection{Utilization Formula}

For this closed network with think time, the theoretical utilization is:

\begin{equation}
U = \frac{K \cdot S}{M \cdot (Z + S)} = \frac{N \cdot S}{M \cdot (1/\lambda + S)}
\end{equation}
...
The empirical and theoretical values closely align, with errors below 3\% for most configurations [Figure~\ref{fig:empirical_vs_theoretical}]:
...
\begin{itemize}
    \item \textbf{Error $<$ 3\%} for $K = 10$ to $500$ users (green bars)
    \item \textbf{Error $\approx$ 7\%} for $K = 1000$ users due to queuing effects
\end{itemize}

\subsubsection{Why Standard Buzen Doesn't Apply}

It is important to note that the standard \textbf{Buzen's convolution algorithm} assumes a pure closed network without think time. In our system:

\begin{itemize}
    \item Think time $Z = 20$s is \textbf{200 times} longer than service time $S = 0.1$s
    \item Users spend 99.5\% of their time ``thinking'' and only 0.5\% being served
    \item Buzen's algorithm (assuming $Z = 0$) would predict saturation for $K > M$
\end{itemize}

Without think time: $U_{Buzen} = \min(1, K/M) = \min(1, 100/10) = 100\%$ (saturated!)

With think time: $U_{actual} = \frac{100 \times 0.1}{10 \times 20.1} = 4.98\%$

The difference is dramatic: \textbf{100\% vs 4.98\%}, which explains why the correct model must account for think time.
...
\begin{equation}
\gamma = \frac{K}{Z + S} \text{ requests/second}
\end{equation}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{images/throughput_vs_users.pdf}
\caption{System throughput vs number of users, showing saturation point at $K_{sat} \approx 2010$}
\label{fig:throughput}
\end{figure}

The saturation point occurs at:
\begin{equation}
K_{sat} = \frac{M \cdot (Z + S)}{S} = \frac{10 \times 20.1}{0.1} = 2010 \text{ users}
\end{equation}
...
\begin{enumerate}
    ...
    \item \textbf{Accurate}: Theoretical predictions match empirical results within 3-7\%
\end{enumerate}
...
\begin{equation}
\boxed{K_{sat} = \frac{M \cdot (Z + S)}{S}}
\end{equation}
\end{tcolorbox}
